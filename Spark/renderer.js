...
.phase-box .gear-btn { position: absolute; top: 16px; right: 16px; }

.phase-screen-title { font-size: 28px; font-weight: 700; color: #FFF; margin-bottom: 28px; text-shadow: 0 0 20px rgba(100,180,255,.6); }

.phase-grid { display: flex; gap: 14px; justify-content: center; flex-wrap: wrap; margin-bottom: 28px; }

.phase-card {
  background: var(--bg-space); border: 2px solid var(--border-soft);
  border-radius: 18px; padding: 20px 14px; cursor: pointer; width: 130px;
  transition: transform .3s cubic-bezier(.34,1.56,.64,1), background .3s, border-color .3s;
  text-align: center;
}
.phase-card:hover:not(.bloqueada) { transform: scale(1.06); background: rgba(100,180,255,.12); border-color: #7AAFE0; }
.phase-card.completa  { border-color: #4ECDC4; }
.phase-card.bloqueada { opacity: .35; cursor: not-allowed; }

.phase-emoji  { font-size: 42px; display: block; margin-bottom: 8px; }
.phase-nome   { color: #FFF; font-weight: 700; font-size: 14px; margin-bottom: 4px; }
.phase-dif    { color: var(--text-muted); font-size: 13px; margin-bottom: 6px; letter-spacing: 1px; }
.phase-status      { font-size: 13px; font-weight: 700; }
.phase-status.ok   { color: #4ECDC4; }
.phase-status.lock { color: rgba(255,255,255,.4); }
...
.block.spin    { background: #CECBF6; color: #3C3489; }
...
#credits-screen { z-index: 95; overflow-y: auto; align-items: flex-start; padding: 40px 16px; }
